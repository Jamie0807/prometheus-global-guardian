...
const COLOR_SCHEME_LABELS: Record<ChartSettings["colorScheme"], string> = {
  default: "默认",
  vibrant: "鲜艳",
  pastel: "柔和",
  monochrome: "单色",
...
const HAZARD_TYPE_LABELS: Record<keyof ChartSettings["customColors"], string> = {
  earthquake: "地震",
  volcano: "火山",
  storm: "风暴",
  flood: "洪水",
...
const CHART_STYLE_LABELS: Record<ChartSettings["chartStyle"], string> = {
  smooth: "平滑",
  sharp: "直线",
};

const FONT_SIZE_LABELS: Record<ChartSettings["fontSize"], string> = {
  small: "小",
  medium: "中",
  large: "大",
};
...
const ChartCustomizationModal: React.FC<ChartCustomizationModalProps> = ({
  ...
  const handleCustomColorChange = (
    hazardType: keyof ChartSettings["customColors"],
    color: string,
  ) => {
    setSettings({
      ...settings,
    ...
    <div className="chart-customization-overlay" onClick={onClose}>
      <div className="chart-customization-modal" onClick={(e) => e.stopPropagation()}>
        <div className="customization-header">
          <div>
            <h3>图表定制</h3>
            <p className="customization-subtitle">个性化配置数据分析面板</p>
          </div>
          <button className="customization-close-btn" onClick={onClose} aria-label="关闭图表定制">
              ...
              {(["default", "vibrant", "pastel", "monochrome", "custom"] as const).map((scheme) => (
                ...
                >
                  <div className="scheme-preview">
                    {scheme !== "custom" &&
                      Object.values(COLOR_SCHEMES[scheme]).map((color, i) => (
                        <div key={i} className="scheme-color" style={{ backgroundColor: color }} />
                      ))}
                    {scheme === "custom" &&
                      Object.values(settings.customColors).map((color, i) => (
                        <div key={i} className="scheme-color" style={{ backgroundColor: color }} />
                      ))}
                  </div>
                  <span className="scheme-name">{COLOR_SCHEME_LABELS[scheme]}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          {settings.colorScheme === "custom" && (
            <div className="customization-section">
              ...
              <div className="custom-colors-grid">
                {(
                  Object.keys(settings.customColors) as Array<keyof ChartSettings["customColors"]>
                ).map((hazardType) => (
                  <div key={hazardType} className="color-picker-item">
                    <label>{HAZARD_TYPE_LABELS[hazardType]}</label>
                    <input
                      type="color"
              ...
              >
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  <path
                    ...
                    strokeLinejoin="round"
                    strokeWidth="2"
                    d="M3 20 Q 6 12, 10 14 T 18 8 L 21 4"
                  />
                </svg>
                {CHART_STYLE_LABELS.smooth}
              </button>
              <button
              ...
              >
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  <path
                    ...
                    strokeLinejoin="round"
                    strokeWidth="2"
                    d="M3 20 L 8 12 L 12 14 L 16 8 L 21 4"
                  />
                </svg>
                {CHART_STYLE_LABELS.sharp}
              </button>
            </div>
          </div>
