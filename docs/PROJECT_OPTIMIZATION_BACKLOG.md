...
- Analytics 前端两阶段的 16 个响应边界已建立运行时解析和明确领域类型；Hazard/通用 HTTP 边界与跨语言契约仍待治理。
...
| P2         | Hazard/HTTP 边界与跨语言契约治理                  | 后续              | Analytics 子批次已完成；继续治理外部数据与模型同步     |
...
| 前端边界类型治理                   | 阶段完成             | 16 个 Analytics 接口由 `unknown` 经运行时解析器进入领域类型；Hazard/HTTP 与跨语言契约仍待治理    |
...
## P2：前端边界类型治理（Analytics 第二阶段子批次已完成）
...
- Hazard Service 与通用 HTTP 外部数据边界仍待治理。
- 导出、第三方组件与旧兼容路径中的动态结构继续按文件范围收敛。
- TypeScript 与 Python 的跨语言契约生成仍待评估，不阻塞已完成的 Analytics 子批次。
...
跨语言共享契约生成在响应模型稳定、手工同步成本明确后评估，不阻塞上述修复。下一批优先推进 **报告下载闭环 → 前端状态归属梳理 → Hazard/通用 HTTP 边界治理与跨语言契约评估**。
