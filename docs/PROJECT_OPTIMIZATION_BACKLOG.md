...
1. **统一灾害事件与图层注册表**：统一事件标识、来源、观测时间、更新时间、几何、严重性、置信度和版本字段；地图、统计、质量和 AI 共用图层元数据。
2. **数据源健康检查**：记录每个数据源的成功率、延迟、最后更新时间、陈旧状态和降级原因，避免页面只显示笼统的分析失败。
3. **PostgreSQL/PostGIS 技术设计**：先完成当前数据、历史快照、分析运行和空间索引的设计；只有确认需要历史查询、回放、审计或长期趋势后再实施数据库迁移。

这三项在下一轮需求拆解时作为 P1 候选任务进入计划。数据库设计属于前置设计，暂不等同于立即引入数据库。
...
| `pnpm run test:services`       | Service、解析器、请求边界、AI 流协议与 HTML 报告回归；当前 194 项。 |
| `pnpm run test:component`      | React Testing Library 组件回归；当前 81 项。                        |
| `pnpm run test:python`         | FastAPI 模型、路由、服务与算法 unittest；当前 49 项。               |
| `pnpm run test:e2e`            | Playwright 桌面端关键流程冒烟。                                     |
| `pnpm run build`               | Vite 生产构建与 BFF 编译。                                          |

本轮验证结果：BFF 84/84、Service 194/194、组件 81/81、Python 49/49、Playwright E2E 1/1；lint、格式、客户端/服务端/契约 TypeScript 类型检查和生产构建均通过，`git diff --check` 已执行。Node 运行时应使用项目声明的 `>=20.19 <21`；当前 Node 24.16.0 会输出 engine warning。
...
- 请求侧 HazardData 与 Analytics 顶层响应信封已双端对齐；4D `data` 内部字段和事件/图层注册表仍需统一。
