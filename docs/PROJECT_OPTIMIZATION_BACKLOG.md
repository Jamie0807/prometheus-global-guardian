...
SSE 断连自动恢复、会话续传、统一灾害事件与图层注册表以及数据源健康检查已经完成。新增的 P1 是多用户账号体系与 AI 会话/记忆持久化；目前只记录需求和阶段，不代表已经完成技术选型或开始实施：

1. **完成认证与持久化技术设计**：确认注册/登录流程、会话安全、数据库与部署边界，并梳理全站页面及 BFF/FastAPI 业务接口的认证要求。
2. **建立用户账号和全站认证**：支持用户注册、登录、退出与会话恢复；服务端验证身份，所有受保护页面和 API 按账号授权。
3. **建立账号隔离的数据持久化**：保存用户的对话及其必要上下文，使刷新后可以恢复；不同账号的数据严格隔离。
4. **加入上下文管理与压缩**：按上下文预算组合当前对话，保留近期消息，并对较早对话生成、更新可持久化摘要。
5. **加入跨对话长期记忆**：从对话形成用户级记忆，在后续对话中按需取回；提供查看、修正、删除和关闭记忆的控制。
6. **补齐验证与运行约束**：覆盖认证边界、跨账号隔离、会话恢复、摘要/记忆生命周期、数据删除和数据库迁移等测试与文档。

数据库产品、认证方式、记忆提炼策略和具体保留期限留待技术设计阶段确定。PostgreSQL/PostGIS 的灾害历史快照、分析运行和空间索引仍是独立的数据设计事项，不因 AI 记忆需求而自动纳入同一迁移。
...
| ------ | ----------------------------------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------ |
| P1     | 用户注册、全站登录与 AI 会话/长期记忆持久化     | 完成技术设计后     | 当前无用户账号和数据库；需要先明确认证、数据隔离、上下文压缩、记忆控制与运行策略。                           |
...
| P2     | 前端功能目录归档与仓库/包结构调整               | 大功能稳定后       | 评估 web/BFF/Python 包边界与依赖；梳理前端 components/features 职责，分阶段归档业务组件并保留兼容入口。      |
...
- 当前没有浏览器用户账号、数据库或持久化会话历史；跨刷新恢复、账号级对话隔离、上下文摘要和跨对话长期记忆列为上述 P1，尚未实施。
...
- SSE 恢复、统一事件/图层注册表与数据源健康检查已完成；下一轮优先完成账号、全站认证与 AI 会话/记忆持久化的技术设计。PostgreSQL/PostGIS 历史数据设计独立评估，避免将不同用途的数据需求未经设计合并。
