...
| `pnpm run test:services`       | Service、解析器、请求边界、AI 流协议、统一灾害事件与 HTML 报告回归；本轮 287 项。 |
...
历史完整验证结果与本轮架构治理验证分别见 `docs/TESTING_BASELINE.md`。本轮 Service 287/287、组件 108/108、Python 57/57、架构检查、lint、格式、客户端/服务端/契约类型检查和构建通过；BFF 与 E2E 分别受本机 `argon2` 原生模块段错误和全局 pnpm 的 Node 版本冲突阻断，不能视为通过。Node 运行时应使用项目声明的 `>=20.19 <21`；当前启动 pnpm 的 Node 24.16.0 会输出 engine warning。
