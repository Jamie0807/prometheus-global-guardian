...
- 运行单元与共享包架构治理已完成：跨语言契约位于 `packages/contracts/`，灾害领域实现位于 `packages/hazard-domain/`；Web 位于 `apps/web/`，BFF 位于 `apps/bff/`，Analytics 位于 `services/analytics/`。Web 与 BFF 通过 `@pgg/hazard-domain` 公共入口使用共享领域包；`shared/hazards/` 保留兼容转导出。架构门禁会检查三个运行单元入口、共享包依赖方向和旧目录残留。
...
| 运行单元与共享包架构治理       | `packages/contracts/` 保存四个语言无关 JSON 契约，`packages/hazard-domain/` 保存 TypeScript 灾害领域实现；Web、BFF、Analytics 分别位于 `apps/web/`、`apps/bff/`、`services/analytics/`，旧 `shared/hazards/` 仅作兼容转导出。`check:architecture` 检查运行单元入口、旧目录残留、包元数据、依赖方向和内部路径引用，并纳入 Node 基线与 CI。 |
...
| `pnpm run test:services`       | Service、解析器、请求边界、AI 流协议、统一灾害事件与 HTML 报告回归；本轮 318 项。 |
...
历史完整验证结果与本轮架构治理验证分别见 `docs/TESTING_BASELINE.md`。本轮 Service 318/318、组件 108/108、架构检查、lint、格式、客户端/服务端/契约类型检查和构建通过；BFF 原生 Node 集成测试仍受本机 `argon2` 原生模块段错误阻断，Docker Node 20 smoke 已通过，不能把宿主机集成测试视为通过。Node 运行时应使用项目声明的 `>=20.19 <21`；当前启动 pnpm 的 Node 24.16.0 会输出 engine warning。
