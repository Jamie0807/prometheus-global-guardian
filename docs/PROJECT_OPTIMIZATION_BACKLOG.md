...
最近核对日期：2026-09-08。本文按当前工作区记录实现状态，包含尚未提交的 Python 管理面治理变更；“已完成”不代表已提交或已部署。
...
| P0     | AI 助手智能路由                   | 已完成                          | 由 BFF 规则路由判断普通模型与 RAG 工作流调用边界       | 第一批   |
...
| P0     | BFF 代理暴露面与请求边界治理      | 阶段完成（BFF / Python 管理面） | 收紧代理、请求边界和管理接口访问范围                   | 第一批   |
...
| P1     | Analytics 结果语义与展示一致性    | 已完成（第二阶段）              | 让预测、风险和质量结果可解释、可比较、可操作           | 第二批   |
| P1     | 统计概览图表数据与坐标一致性      | 已完成                          | 防止图表使用随机强度、坐标裁剪和记录索引错位           | 第二批   |
| P1     | 外部数据源时效性与韧性            | 待开始                          | 提升刷新稳定性、降级可见性和数据新鲜度                 | 第二批   |
| P1     | AI 流式会话生命周期治理           | 待开始                          | 支持取消、输入限额、断流处理和成本控制                 | 第二批   |
| P1     | 统一灾害数据入口与来源级状态      | 待开始                          | 统一数据调度并保留来源失败与新鲜度信息                 | 第二批   |
| P1     | 实现自动刷新与请求竞态保护        | 待开始                          | 防止旧响应覆盖新状态并完善请求清理                     | 第二批   |
| P1     | 前后端错误信息与调试信息分级      | 待开始                          | 分离用户消息、日志和开发诊断                           | 第二批   |
...
| P2     | 运行时无关文件与仓库卫生          | 待开始                          | 明确旧副本、审查产物与锁文件保留策略                   | 后续     |
| P2     | 依赖、镜像和配置持续审计          | 待开始                          | 建立依赖、镜像和配置的持续检查                         | 后续     |
...
| Python 管理面边界              | 已落地（尚未提交）   | 管理接口令牌保护、显式 CORS 来源和 Compose 本地端口绑定；公开 API 治理仍待推进              |
...
2. 在已完成 BFF 与 Python 管理面边界的基础上，推进公开 Analytics API 身份策略、共享限流和其余错误信息分级。
3. 修复地图 Popup 输出安全问题，并补齐报告下载闭环。
4. 统一灾害数据入口，增加来源状态、自动刷新、取消和竞态保护。
5. 地图模块拆分已完成；在现有 Hook 边界内推进 Popup 安全、来源状态、自动刷新和生命周期测试。
...
完成项以“已完成优化项”汇总为准；优先推进尚未闭环的地图 Popup 安全和报告下载，其后继续数据来源、请求生命周期、Python 可靠性与 CI 门禁建设。
