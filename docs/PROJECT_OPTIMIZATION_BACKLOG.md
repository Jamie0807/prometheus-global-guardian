...
SSE 断连自动恢复、会话续传以及统一灾害事件与图层注册表已经完成。以下事项来自开源项目对比和当前运行问题，按“先稳定契约，再建设数据能力”的顺序执行：

1. **数据源健康检查**：记录每个数据源的成功率、延迟、最后更新时间、陈旧状态和降级原因，避免页面只显示笼统的分析失败。
2. **PostgreSQL/PostGIS 技术设计**：先完成当前数据、历史快照、分析运行和空间索引的设计；只有确认需要历史查询、回放、审计或长期趋势后再实施数据库迁移。

这两项在下一轮需求拆解时作为 P1 候选任务进入计划。数据库设计属于前置设计，暂不等同于立即引入数据库；复杂几何和历史回放也不因统一事件模型完成而自动纳入范围。
...
最近完整验证结果：BFF 88/88、Service 235/235、组件 84/84、Python 56/56、Playwright E2E 1/1；lint、格式、客户端/服务端/契约 TypeScript 类型检查和生产构建均通过，`git diff --check` 已执行。Node 运行时应使用项目声明的 `>=20.19 <21`；当前 Node 24.16.0 会输出 engine warning。
...
- SSE 恢复与统一事件/图层注册表已完成；下一轮按“数据源健康检查，最后评估数据库落地”的顺序拆解任务。
