...
最近核对日期：2026-09-24。本文档以当前工作区代码、自动化脚本、GitHub Actions 与最近提交为准；账号/AI 持久化实现已合并到 `main`，尚未部署。
...
| `pnpm run test:services`       | Service、解析器、请求边界、AI 流协议、统一灾害事件与 HTML 报告回归；当前 245 项。 |
| `pnpm run test:component`      | React Testing Library 组件回归；当前 105 项。                                     |
...
| P2     | 全项目运行单元与共享包架构治理         | 大功能稳定后       | 梳理 Web、BFF、Python 服务与共享契约边界；评估 workspace 分层和任务编排。                             |
