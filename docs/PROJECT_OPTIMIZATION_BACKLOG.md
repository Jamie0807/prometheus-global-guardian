...
最近核对日期：2026-09-19。本文档以当前工作区代码、自动化测试脚本、GitHub Actions 与最近提交为准；已完成表示代码和验证入口已经落地，不表示项目已经部署。
...
SSE 断连自动恢复与会话续传已经完成。以下事项来自开源项目对比和当前运行问题，按“先稳定契约，再建设数据能力”的顺序执行：

1. **Analytics 响应契约版本化**：统一 `schemaVersion`、`requestId`、`generatedAt`、`modelVersion`、`inputSnapshotId`、`warnings` 和错误信封，并补充跨语言契约测试。
2. **统一灾害事件与图层注册表**：统一事件标识、来源、观测时间、更新时间、几何、严重性、置信度和版本字段；地图、统计、质量和 AI 共用图层元数据。
3. **数据源健康检查**：记录每个数据源的成功率、延迟、最后更新时间、陈旧状态和降级原因，避免页面只显示笼统的分析失败。
4. **PostgreSQL/PostGIS 技术设计**：先完成当前数据、历史快照、分析运行和空间索引的设计；只有确认需要历史查询、回放、审计或长期趋势后再实施数据库迁移。

这四项在下一轮需求拆解时作为 P1 候选任务进入计划。数据库设计属于前置设计，暂不等同于立即引入数据库。
...
| `pnpm run test:services`       | Service、解析器、请求边界、AI 流协议与 HTML 报告回归；当前 188 项。 |
| `pnpm run test:component`      | React Testing Library 组件回归；当前 81 项。                        |
...
本轮验证结果：BFF 78/78、Service 188/188、组件 81/81、Python 44/44、Playwright E2E 1/1；lint、格式、客户端/服务端/契约 TypeScript 类型检查、生产构建和差异检查均通过。Node 运行时应使用项目声明的 `>=20.19 <21`；其他 Node 版本会输出 engine warning。
...
| ------ | ----------------------------------------------- | ------------------ | ------------------------------------------------------------------------------------------------------------ |
| P1     | Analytics 响应契约版本化与跨语言 4D 输出扩展    | 下一步优先实施     | 统一成功和失败信封、版本、requestId、模型版本和输入快照；补固定跨语言响应样本或代码生成，防止响应字段漂移。  |
| P1     | 统一灾害事件模型与图层注册表                    | 契约设计阶段       | 统一来源、时间、几何、严重性、置信度和图层元数据，避免地图、统计、质量和 AI 各自维护字段映射。               |
| P1     | 数据源健康检查与新鲜度                          | 数据采集改造前     | 记录成功率、延迟、最后更新时间、陈旧状态和降级原因；不改变现有多源降级语义。                                 |
...
- 请求侧 HazardData 已双端对齐；Analytics 响应、4D 输出和生成式跨语言类型仍未统一，已列为下一步 P1。
- 灾害事件和图层元数据尚未形成跨地图、统计、质量和 AI 的统一注册表，新增数据源前应先确定字段、时间和几何语义。
- 数据源目前具备来源级状态和陈旧标记，但尚未形成统一的健康指标、延迟记录和降级原因契约。
...
- SSE 恢复已完成；下一轮按“Analytics 契约、事件/图层注册表和数据源健康检查，最后评估数据库落地”的顺序拆解任务。
