...
### 11. React.memo 是什么？
**核心概念**：**React.memo** 是一个高阶组件（HOC），用于**缓存组件的渲染结果**。如果组件的 props 没有发生变化，React 将跳过渲染组件的操作，直接复用最近一次渲染的结果。

**解决了什么问题？**
*   **避免无效渲染**：当父组件重新渲染时，默认情况下所有子组件都会重新渲染。如果子组件的 props 其实没变，这种重渲染就是浪费。`React.memo` 通过**浅比较（Shallow Compare）** props 来决定是否更新。

**在本项目中的应用**：
简历中提到 *"使用 React.memo 优化组件重渲染"*。
*   **场景**：`StatisticsCard` 组件。
*   **问题**：地图上的灾害数据（`hazards`）每 5 分钟更新一次，或者用户拖动地图时父组件可能会更新。但某些统计卡片（如“总灾害数”）如果数值没变，就不应该重新渲染 DOM。
*   **代码示例**：
    ```tsx
    // 使用 React.memo 包裹组件
    const StatisticsCard = React.memo(({ title, value }) => {
      return (
        <div className="card">
          <h3>{title}</h3>
          <p>{value}</p>
    ...
    ```
*   **配合 useCallback**：如果传递给子组件的是函数（如 `onClick`），必须配合 `useCallback` 使用，否则每次父组件渲染都会生成新的函数引用，导致 `React.memo` 失效。

### 12. Tree Shaking（摇树优化）是什么？
...
### 16. HMR (Hot Module Replacement) 热更新是什么？
