...
您好，我叫 xxx，硕士毕业于英国利物浦大学数据科学与人工智能专业，有 5 年北京一线前端经验，目前在伦敦做全栈工程师。

我的技术方向以前端为核心，向可视化、全栈和 AI 工程化多侧延伸。前端上我熟悉 React/Vue 和工程化链路；可视化上有 WebGL 和 Mapbox GL JS 的实战经验；后端能用 Node.js 和 Python 独立交付；AI 方向做过 LangChain/LangGraph 工作流落地，也在持续实践 vibe coding。

我的长期方向是往 AI coding 研发工具走，重点做“工程可落地”的能力，比如需求拆解、代码生成、测试校验和质量门禁的工具化。

最近的项目中我从0到1主导了基于webGL的渲染器引擎的整体重构。
原方案在高密度点位下性能明显下降。我主导做了渲染分层、增量更新和 Worker 线程拆分，最终把高数据量场景的流畅度和稳定性都提上来了。

第二个是主导从零设计了一套 AI 工作流执行引擎。这个项目最大的挑战是，复杂的多步骤分支流程无法靠单次模型调用稳定承载。为此我从零主导设计了 AI 工作流执行引擎，统一了流程编排、状态流转和模型接入，并以 SaaS API 形式沉淀为内部通用能力，最终把 AI 场景的交付效率明显拉高。

整体来说，我比较擅长把复杂问题工程化拆解并落地，既能抓前端体验，也能兼顾系统架构和业务结果。谢谢！
