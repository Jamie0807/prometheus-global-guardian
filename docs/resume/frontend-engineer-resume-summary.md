...
> **💡 这是 RAG 吗？**
>
> **不是 RAG，是更轻量的动态上下文注入（Dynamic Context Injection）**，两者有本质区别：
>
> | | **RAG** | **本项目** |
> |---|---|---|
> | **数据存储** | 向量数据库（Pinecone / Chroma 等） | 无，直接用内存中的实时数据 |
> | **检索步骤** | 提问 → Embedding → 向量相似度检索 → 召回文档片段 | 无检索，直接将平台实时数据结构化拼入 Prompt |
> | **数据来源** | 离线索引的文档库（PDF、网页等） | 平台实时抓取的灾害事件（`hazards` 数组） |
> | **适合场景** | 大量静态知识库问答 | 少量、结构化、实时变化的数据 |
>
> **本项目不用 RAG 的三个理由**：
> 1. **数据量小且结构化**：灾害事件本就是结构化字段（类型/数量/坐标），直接注入比向量检索更精准
> 2. **实时性要求高**：数据 5 分钟刷新一次，RAG 需要先建索引，维护向量库成本远高于收益
> 3. **无知识库需求**：不需要从大量文档召回片段，LLM 自身的地理/灾害领域知识已足够
>
> **若未来升级为 RAG**（接入历史灾害报告、应急预案文档等大量非结构化知识库）：可用 **LangChain.js + Pinecone** 构建向量检索管道，或直接使用 OpenAI Assistants API 的 File Search 功能。
