    ...
    signals: [
      {
        label: "灾害类型",
        pattern:
          /地震|火山|洪水|洪涝|风暴|台风|飓风|野火|森林火灾|火灾|干旱|海啸|滑坡|泥石流|雷暴|极端天气|earthquake|volcano|flood|storm|wildfire|drought|tsunami/i,
      },
      {
        label: "灾害研判",
        pattern: /灾害|灾情|风险评估|风险分析|灾害趋势|预警|次生灾害|减灾|抗灾/i,
      },
    ],
  },
  {
    ...
    signals: [
      {
        label: "实时监控",
        pattern: /实时|当前|平台数据|监控数据|活跃事件|态势|数据快照|现在有多少/i,
      },
    ],
  },
];
...
export function routeAIRequest(
  ...
  if (
    disasterContext &&
    Number(disasterContext.total) > 0 &&
    /分析|总结|概况|报告|评估|研判|趋势|风险/i.test(content)
  ) {
    return {
      target: "workflow",
      reason: "live_context",
