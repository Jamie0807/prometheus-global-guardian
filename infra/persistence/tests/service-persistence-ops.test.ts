...
describe("persistence command boundaries", () => {
  ...
  it("includes both Task 1 utility files in the format check", async () => {
    ...
    expect(packageJson.scripts["format:check"]).toContain("infra/persistence/backup-utils.mjs");
    expect(packageJson.scripts["format:check"]).toContain("infra/persistence/backup-utils.d.ts");
  });
